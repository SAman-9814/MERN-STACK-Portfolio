...
const formatMessageText = (text) => {
    ...
    return lines.map((line, lineIdx) => {
        ...
        if (line.trim() === '' && lineIdx < lines.length - 1) {
            return <span key={lineIdx} className="block min-h-[1.2em]" />;
        }
...
export default function Chatbot() {
    ...
    const [messages, setMessages] = useState(() => {
        ...
        return [
            { 
                from: 'bot', 
                text: "Hi! I'm ARIA 🤖 — Aman's Real-time Intelligent Assistant. Ask me anything about Aman's work, skills, or projects!",
    ...
    const getLocalReply = (text) => {
        ...
        if (lower.includes('who') || lower.includes('name') || lower.includes('about')) return FAQ[0].a;
        if (lower.includes('stack') || lower.includes('tech') || lower.includes('skill')) return FAQ[1].a;
        if (lower.includes('ai') || lower.includes('automation') || lower.includes('n8n') || lower.includes('openai')) return FAQ[2].a;
        if (lower.includes('work') || lower.includes('job') || lower.includes('experience') || lower.includes('company')) return FAQ[3].a;
        if (lower.includes('contact') || lower.includes('email') || lower.includes('reach') || lower.includes('whatsapp')) return FAQ[4].a;
        if (lower.includes('resume') || lower.includes('cv') || lower.includes('download')) return FAQ[5].a;
        if (lower.includes('hi') || lower.includes('hello') || lower.includes('hey')) return "Hey! 👋 Great to meet you! How can I help you learn more about Aman?";
        return "I'm not sure about that, but feel free to email Aman at sah99017@gmail.com 😊";
    };

    const triggerChatApi = async (text) => {
        setTyping(true);
        ...
        try {
            ...
            if (res.success && res.reply) {
                ...
                setMessages(prev => [...prev, { 
                    from: 'bot', 
                    text: getLocalReply(text),
                    time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                }]);
            }
        } catch (err) {
            clearTimeout(timeoutId);
            console.error("AI chatbot API error, using local fallback:", err);
            setTyping(false);
            
            const fallbackText = err.name === 'AbortError'
                ? "Connection timed out. Here is a quick answer from my local database:\n\n" + getLocalReply(text)
                : getLocalReply(text);
    ...
    const handleReset = () => {
        ...
        sessionStorage.setItem('aria_chat_history', JSON.stringify(defaultMsg));
    };
            ...
            <AnimatePresence>
                {open && (
                    ...
                    >
                        ...
                        <div className="relative flex items-center justify-between px-5 py-4 header-sweep">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={handleReset}
                                    title="Reset chat"
                                    className="w-7 h-7 rounded-full bg-white/10 hover:bg-white/25 flex items-center justify-center text-white/80 hover:text-white transition-all duration-200 hover:scale-110">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                                        <path d="M9 3v1H4v2h1v13a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6h1V4h-5V3H9zm2 14H9v-8h2v8zm4 0h-2v-8h2v8z"/>
                                    </svg>
                                </button>
                        ...
                        {/* Messages Container */}
                        <div data-lenis-prevent className="h-64 overflow-y-auto px-4 py-4 flex flex-col gap-3">
                            {messages.map((msg, i) => (
                                <motion.div
                                    key={i}
                                ...
                                >
                                    ...
                                    <div className="flex flex-col max-w-[75%]">
                                        <div
                                            className={`px-4 py-2.5 rounded-2xl text-xs font-Ovo leading-relaxed
                                                ${msg.from === 'user'
                                        ...
                                        {msg.time && (
                                            <span className={`text-[9px] text-white/40 mt-1 px-1 font-Ovo ${msg.from === 'user' ? 'text-right' : 'text-left'}`}>
                                                {msg.time}
                                            </span>
                                        )}
                                    </div>
